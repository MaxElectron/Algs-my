...
public:
    ...
    [[maybe_unused]] explicit Fenwick(const std::vector<T>& base);

    [[maybe_unused]] void change(long long index, const T& value);

    [[maybe_unused]] [[maybe_unused]] void set(long long index, const T& value);
...
template<typename T, class Arithmetic>
[[maybe_unused]] Fenwick<T, Arithmetic>::Fenwick(const std::vector<T>& base) {
    data = std::vector<T> (base.size(), foo.neutral());

    data[0] = base[0];
    for (int i = 1; i < base.size(); ++i) {
        data[i] = data[i-1] + base[i];
    }

    for (int i = base.size()-1; i > 0; --i) {
        int next = (i & (i + 1)) - 1;
        if (next >= 0) data[i] -= data[next];
    }

}

template<typename T, class Arithmetic>
[[maybe_unused]] void Fenwick<T, Arithmetic>::change(long long index, const T& value) {
...
template<typename T, class Arithmetic>
[[maybe_unused]] void Fenwick<T, Arithmetic>::set(long long index, const T& value) {
